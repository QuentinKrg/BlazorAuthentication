...
namespace LoroWasmAuthentication.Server.Controllers
{
    [Route("[controller]")]
    [ApiController]
    public class UserController : ControllerBase
    {
        public const string GROUP_USER = "DPT_DOSI";
        public const string GROUP_ADMIN = "NOM_DU_GROUPE_ADMIN";
        ...
        {
            LoroLdapManager.LoRoLdap ldap = new LoroLdapManager.LoRoLdap();
            if (!ldap.Authentication(authenticationRequest.UserLogin, authenticationRequest.Password))
            {
                return NotFound();
            }
            else
            {
                ...
                if (ldap.IsMemberOf(authenticationRequest.UserLogin, GROUP_USER))
                    claimRoles.Add(GROUP_USER);

                if (ldap.IsMemberOf(authenticationRequest.UserLogin, GROUP_ADMIN))
                ...
                try
                {
                    User user = GetUserByUserLogin(authenticationRequest.UserLogin, ldap).Result;
        ...
        protected async Task<User> GetUserByUserLogin(string userLogin, LoroLdapManager.LoRoLdap ldap)
        {
            Dictionary<string, string> details = ldap.GetUserDetails(userLogin);
            var user = new User();
            user.FirstName = details.GetValueOrDefault("givenName", "");
            user.LastName = details.GetValueOrDefault("sn", "");
